...
class WechatAuthenticate
{
    ...
    public function handle($request, Closure $next, $scopes = null)
    {
        ...
        if (!session('wechat.oauth_user') || $this->needReauth($scopes)) {
            ...
            $path = "http://wx.dev.yunchongba.com/" . $request->path(). '?type=xxx' .(empty($request->query()) ? '' : '&'.http_build_query($request->query()));
            return $this->wechat->oauth->scopes($scopes)->redirect($path);
        }
    ...
    protected function getTargetUrl($request)
    {
        ...
        // return "http://wx.dev.yunchongba.com/" . $request->path().(empty($queries) ? '' : '?'.http_build_query($queries));
    }
